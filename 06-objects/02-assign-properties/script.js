...
(() => {
    ...
    document.getElementById("run").addEventListener("click", function() {
        ...
        for (var i = 0; i < length; i++) {         
            if (!computers[i].hasOwnProperty("available")) { // object.propName !== undefined
                computers[i].available = defaultProps.available;
            }

            if (!computers[i].hasOwnProperty("os")) { // object.propName !== undefined
                computers[i].os = defaultProps.os;
            }

            if (!computers[i].hasOwnProperty("user")) { // object.propName !== undefined
